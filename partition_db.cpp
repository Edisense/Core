...
bool PartitionDB::put(device_t device_id, time_t timestamp, time_t expiration, void *data, size_t datalen)
{
	...
	sql_len = ssprintf(sql, "SELECT * FROM stored_values WHERE (device_id == %d AND timestamp == %d);", device_id, timestamp);
	stmt = NULL;
	...
	while (true)
	{
		...
		else if (result == SQLITE_ROW)
		{
			...
			void *row_data = sqlite3_column_blob(stmt, 3);
			assert(row_data);
			if (!compareBuffer)
			{
				sqlite3_finalize(stmt);
	...
	stmt = NULL;
	sql_len = ssprintf(sql, "INSERT INTO stored_values VALUES(%d, %d, %d, ?);", device_id, timestamp, expiration);
	result = sqlite3_prepare_v2(db, sql, sql_len, &stmt, NULL);
	if (result != SQLITE_OK)
	{
		return false;
...
std::list<struct data> PartitionDB::get(device_t device_id, time_t min_timestamp, time_t max_timestamp)
{
	...
	sql_len = ssprintf(sql, "SELECT * FROM stored_values" \
	...
	while (true)
	{
		...
		else if (result == SQLITE_ROW)
		{
			...
			void *data = sqlite3_column_blob(stmt, 3);
...
void PartitionDB::remove(time_t timestamp)
{
	char sql[256];
	int sql_len;
	int result;
	...
	sql_len = ssprintf(sql, "DELETE FROM stored_values WHERE (timestamp < %d);", timestamp);
	result = sqlite3_prepare_v2(db, sql, sql_len, &stmt, NULL);
	if (result != SQLITE_OK)
	{
		sqlite3_finalize(stmt);
